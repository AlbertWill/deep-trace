...
2. MCP server mode regression — importing mcp_server.py must not raise, and
   ...
   confirms that the server-side MCP plugin (deep-trace-mcp) is unaffected
...
- TODO(v1): Add a live smoke test that spawns `deep-trace-mcp` as a stdio
...
# ---------------------------------------------------------------------------
# MCP server mode regression (deep-trace-mcp plugin)
# ---------------------------------------------------------------------------
