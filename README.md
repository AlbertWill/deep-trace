...
<p align="center">
  <img src="assets/icon.png" width="120" alt="Deep-Trace Logo"/>
</p>

<h1 align="center">Deep-Trace: Your Personal Trading Agent</h1>
...
<p align="center">
  ...
  <a href="https://pypi.org/project/deep-trace-ai/"><img src="https://img.shields.io/pypi/v/deep-trace-ai?style=flat&logo=pypi&logoColor=white" alt="PyPI"></a>
...
<p align="center">
  <a href="https://deeptrace.wiki/">Website</a> &nbsp;&middot;&nbsp;
  <a href="https://deeptrace.wiki/docs/">Docs</a> &nbsp;&middot;&nbsp;
...
<p align="center">
  <a href="#-quick-start"><img src="assets/pip-install.svg" height="45" alt="pip install deep-trace-ai"></a>
</p>
...
- **2026-05-20** 🔬 **Hypothesis Registry CLI**: Closes the CLI side of the Hypothesis Registry shipped backend-only on 2026-05-16. `deep-trace hypothesis list` prints a Rich table or JSON (`--status` filter, `--limit`); `show <id>` renders a detail panel including linked run cards; `invalidate <id> --note "..."` flips status to `rejected` while preserving prior invalidation notes when `--note` is omitted. Honors the existing `DEEP_TRACE_HYPOTHESES_PATH` env override and adds a per-invocation `--path`. 22 new tests cover wiring, JSON output, status filter, limit, missing-id errors, and note persistence.
...
- **2026-05-15** 🪪 The run detail page now surfaces the Trust Layer run card alongside metrics and artifacts, completing the UI side of the `run_card.json` work landed on 2026-05-12. `PersistentMemory.add()` was also hardened on length, empty/whitespace-only names, and C0/C1 control bytes from the #108/#109/#110 triage ([#112](https://github.com/HKUDS/Deep-Trace/pull/112), thanks @Teerapat-Vatpitak).
- **2026-05-14** 🌐 the public wiki is now live at [deeptrace.wiki](https://deeptrace.wiki/) with docs, tutorials, Research Lab, and Alpha Library sections deployed through Cloudflare Pages. Persistent memory is also inspectable from the CLI via `deep-trace memory list/show/search/forget` ([#102](https://github.com/HKUDS/Deep-Trace/pull/102), thanks @Teerapat-Vatpitak), and memory tokenization/slugs now support Thai, Arabic, Hebrew, and Cyrillic text ([#104](https://github.com/HKUDS/Deep-Trace/pull/104)).
- **2026-05-13** 🧭 Swarm runs now ground workers with fetched market data and cleaner persisted reports ([#93](https://github.com/HKUDS/Deep-Trace/pull/93), [#84](https://github.com/HKUDS/Deep-Trace/pull/84)).
- **2026-05-12** 🧾 Backtests now emit `run_card.json` and `run_card.md` alongside artifacts for reproducible research runs.
- **2026-05-11** 🧭 **Memory slugs, swarm accounting, and CLI preflight**: Persistent memory now preserves CJK characters when generating file slugs, preventing silent filename collisions for Chinese/Japanese/Korean notes ([#95](https://github.com/HKUDS/Deep-Trace/pull/95), thanks @voidborne-d). Swarm run totals now prefer provider-reported token usage with the existing estimate fallback ([#94](https://github.com/HKUDS/Deep-Trace/pull/94), thanks @Teerapat-Vatpitak), and the CLI run UI gained a startup preflight check for common environment issues ([#96](https://github.com/HKUDS/Deep-Trace/pull/96), thanks @ykykj).
- **2026-05-10** 🧱 **Regression guardrails + run metadata**: Memory recall now treats underscores as token boundaries, so snake_case saved memories such as `mcp_wiring_test` match natural-language queries like "mcp wiring" ([#87](https://github.com/HKUDS/Deep-Trace/pull/87), thanks @hp083625). The MCP server has a subprocess smoke test covering initialize → `tools/list` → `tools/call` to guard the first-call deadlock path ([#86](https://github.com/HKUDS/Deep-Trace/pull/86)), while low-risk hardening landed for Windows path-sensitive tests, API best-effort exception handling, backtest `run_dir` allowed-root validation, and SwarmRun provider/model metadata ([#88](https://github.com/HKUDS/Deep-Trace/pull/88), [#90](https://github.com/HKUDS/Deep-Trace/pull/90), [#91](https://github.com/HKUDS/Deep-Trace/pull/91), [#92](https://github.com/HKUDS/Deep-Trace/pull/92), thanks @Teerapat-Vatpitak).
- **2026-05-09** 🛡️ **API path hardening + MCP server stability**: API run/session routes now validate path IDs before lookup, rejecting malformed newline-containing parameters and pinning the behavior in the auth/security regression suite ([#80](https://github.com/HKUDS/Deep-Trace/pull/80), thanks @SJoon99). The MCP server now pre-warms the tool registry on the main thread before serving `tools/call`, avoiding a first-call deadlock in lazy tool discovery ([#85](https://github.com/HKUDS/Deep-Trace/pull/85), thanks @Teerapat-Vatpitak). The Vite dev proxy also honors `VITE_API_URL` for non-default backend targets ([#82](https://github.com/HKUDS/Deep-Trace/pull/82), thanks @voidborne-d).
- **2026-05-08** 🧾 **Tushare statement fields in filters**: A-share daily backtests can now request PIT-safe financial statement fields through `fundamental_fields`, so signal engines can screen on `income_total_revenue`, `income_n_income`, `balancesheet_total_hldr_eqy_exc_min_int`, `fina_indicator_roe`, and similar table-prefixed columns after their announcement/disclosure dates ([#76](https://github.com/HKUDS/Deep-Trace/pull/76), thanks @mrbob-git). Follow-up hardening makes explicit statement-field requests fail fast if Tushare enrichment cannot run, instead of silently falling back to raw price bars ([#77](https://github.com/HKUDS/Deep-Trace/pull/77)).
- **2026-05-07** 📈 **Tushare fundamentals + community triage**: Added a point-in-time `TushareFundamentalProvider` contract for fundamental research workflows, with regression coverage for the project `TUSHARE_TOKEN` environment path ([#74](https://github.com/HKUDS/Deep-Trace/pull/74)). Community triage also clarified that Deep-Trace keeps rapid iteration focused on one UI language for now, avoids adding redundant search dependencies while DuckDuckGo-backed `web_search` is already bundled, and treats unofficial hosted deployments as untrusted places for API keys or data-source tokens.
- **2026-05-06** 🚀 **v0.1.7 released** ([Release notes](https://github.com/HKUDS/Deep-Trace/releases/tag/v0.1.7), `pip install -U deep-trace-ai`): Security-boundary hardening is now published on PyPI and ClawHub, covering safer API/read/upload/file/URL/generated-code/shell-tool/Docker defaults while keeping localhost CLI/Web UI workflows low-friction. This cycle also includes Web UI Settings, correlation heatmap, OpenAI Codex OAuth, A-share pre-ST filtering, interactive CLI UX, swarm preset inspection, dividend analysis, dev workflow polish, and audited frontend build-dependency floors. Thanks to the 0.1.7 contributors and to lemi9090 (S2W) for coordinated security validation.
...
- **2026-05-01** 🔥 **Correlation heatmap + OpenAI Codex OAuth + A-share pre-ST filter**: New correlation dashboard/API computes rolling return correlations and renders an ECharts heatmap for portfolio and symbol analysis ([#64](https://github.com/HKUDS/Deep-Trace/pull/64)). OpenAI Codex provider support now uses ChatGPT OAuth via `deep-trace provider login openai-codex`, with Settings metadata and adapter regression tests ([#65](https://github.com/HKUDS/Deep-Trace/pull/65)). Added and hardened the `ashare-pre-st-filter` skill for A-share ST/*ST risk screening, including Sina penalty relevance filtering so securities-account mentions do not inflate E2 counts ([#63](https://github.com/HKUDS/Deep-Trace/pull/63)).
- **2026-04-30** ⚙️ **Web UI Settings + validation CLI hardening**: New Settings page for LLM provider/model, base URL, reasoning effort, and data source credentials, backed by local/auth-protected settings APIs and data-driven provider metadata ([#57](https://github.com/HKUDS/Deep-Trace/pull/57)). Also hardens `python -m backtest.validation <run_dir>` so missing, blank, malformed, non-existent, and non-directory inputs fail with clear operator-facing messages before validation starts ([#60](https://github.com/HKUDS/Deep-Trace/pull/60)).
- **2026-04-28** 🚀 **v0.1.6 released** (`pip install -U deep-trace-ai`): Fixes `deep-trace --swarm-presets` returning empty after `pip install` / `uv tool install` ([#55](https://github.com/HKUDS/Deep-Trace/issues/55)) — preset YAMLs now bundled inside the `src.swarm` package and pinned by a 6-test regression suite. Plus AKShare loader correctly routes ETFs (`510300.SH`) and forex (`USDCNH`) to the right endpoints with hardened registry fallback. Rolls up everything since v0.1.5: benchmark comparison panel, `/upload` streaming + size limits, Futu loader (HK + A-share), vnpy export skill, security hardening, frontend lazy loading (688KB → 262KB).
- **2026-04-27** 📊 **Benchmark panel + upload safety**: Backtest output now ships a benchmark comparison panel (ticker / benchmark return / excess return / information ratio) with yfinance-backed resolution for SPY, CSI 300, etc. ([#48](https://github.com/HKUDS/Deep-Trace/issues/48)). Plus `/upload` streams the request body in 1 MB chunks and aborts past `MAX_UPLOAD_SIZE`, bounding memory under oversized/malformed clients ([#53](https://github.com/HKUDS/Deep-Trace/pull/53)) — pinned by a 4-case regression suite.
- **2026-04-22** 🛡️ **Hardening + new integrations**: Path containment enforced in `safe_path` + journal/shadow tool sandbox, `MANIFEST.in` ships `.env.example` / tests / Docker files in sdist, route-level lazy loading shrinks frontend initial bundle 688KB → 262KB. Plus Futu data loader for HK & A-share equities ([#47](https://github.com/HKUDS/Deep-Trace/pull/47)) and vnpy CtaTemplate export skill ([#46](https://github.com/HKUDS/Deep-Trace/pull/46)).
- **2026-04-21** 🛡️ **Workspace + docs**: Relative `run_dir` normalized to active run dir ([#43](https://github.com/HKUDS/Deep-Trace/pull/43)). README usage examples ([#45](https://github.com/HKUDS/Deep-Trace/pull/45)).
- **2026-04-20** 🔌 **Reasoning + Swarm**: `reasoning_content` preserved across all `ChatOpenAI` paths — Kimi / DeepSeek / Qwen thinking work end-to-end ([#39](https://github.com/HKUDS/Deep-Trace/issues/39)). Swarm streaming + clean Ctrl+C ([#42](https://github.com/HKUDS/Deep-Trace/issues/42)).
...
- **2026-04-15** 🤖 **Z.ai + MiniMax**: Z.ai provider ([#35](https://github.com/HKUDS/Deep-Trace/pull/35)), MiniMax temperature fix + model update ([#33](https://github.com/HKUDS/Deep-Trace/pull/33)). 13 providers.
- **2026-04-14** 🔧 **MCP Stability**: Fixed backtest tool `Connection closed` error on stdio transport ([#32](https://github.com/HKUDS/Deep-Trace/pull/32)).
...
- **2026-04-11** 🛡️ **Reliability & DX**: `deep-trace init` .env bootstrap ([#19](https://github.com/HKUDS/Deep-Trace/pull/19)), preflight checks, runtime data-source fallback, hardened backtest engine. Multi-language README ([#21](https://github.com/HKUDS/Deep-Trace/pull/21)).
- **2026-04-10** 📦 **v0.1.4**: Docker fix ([#8](https://github.com/HKUDS/Deep-Trace/issues/8)), `web_search` MCP tool, 12 LLM providers, `akshare`/`ccxt` deps. Published to PyPI and ClawHub.
...
Deep-Trace is an open-source research workspace for turning finance questions into runnable analysis. It connects natural-language prompts to market-data loaders, strategy generation, backtest engines, reports, exports, and persistent research memory.
...
deep-trace run -p "Backtest a BTC-USDT 20/50 moving-average strategy for 2024, summarize return and drawdown, then export the report"

# Bench a pre-built alpha zoo (one line)
deep-trace alpha bench --zoo gtja191 --universe csi300 --period 2018-2025 --top 20
```

```bash
deep-trace --upload trades_export.csv
deep-trace run -p "Analyze my trading behavior, extract my shadow strategy, and compare it with my actual trades"
```
...
<sub>Plus 20+ additional specialist presets — run deep-trace --swarm-presets to explore all.
...
Run `deep-trace alpha list` to browse, `deep-trace alpha show <id>` for formulas + source, `deep-trace alpha bench --zoo X --universe Y --period Z` to score a whole zoo.
...
```bash
deep-trace init
deep-trace run -p "Backtest a BTC-USDT 20/50 moving-average strategy for 2024 and summarize return and drawdown"
```

> **Package name vs commands:** The PyPI package is `deep-trace-ai`. Once installed, you get three commands:
>
> | Command | Purpose |
> |---------|---------|
...
```bash
deep-trace init              # interactive .env setup
deep-trace                   # launch CLI
deep-trace serve --port 8899 # launch web UI
deep-trace-mcp               # start MCP server (stdio)
```
...
- **Docker** for Path A
- OpenAI Codex can also be used with ChatGPT OAuth: set `LANGCHAIN_PROVIDER=openai-codex`, then run `deep-trace provider login openai-codex`. This does not use `OPENAI_API_KEY`.
...
```bash
git clone https://github.com/HKUDS/Deep-Trace.git
cd Deep-Trace
...
deep-trace                       # Launch interactive TUI
```
...
deep-trace serve --port 8899     # FastAPI serves dist/ as static files
```
...
```bash
npx clawhub@latest install deep-trace --force
```
...
| `DEEP_TRACE_ENABLE_SHELL_TOOLS` | No | Explicit opt-in for shell-capable tools in remote API/MCP-SSE style deployments |
| `DEEP_TRACE_ALLOWED_FILE_ROOTS` | No | Extra comma-separated roots for document and broker-journal imports |
| `DEEP_TRACE_ALLOWED_RUN_ROOTS` | No | Extra comma-separated roots for generated-code run directories |
...
Deep-Trace is a tool-heavy agent — skills, backtests, memory, and swarms all flow through tool calls. Model choice directly decides whether the agent *uses* its tools or fabricates answers from training data.
...
```bash
deep-trace               # interactive TUI
deep-trace run -p "..."  # single run
deep-trace serve         # API server
deep-trace alpha list    # browse 452 pre-built alphas; show / bench / compare / export-manifest sub-commands available
```
...
```bash
deep-trace run -p "Backtest BTC-USDT MACD strategy, last 30 days"
deep-trace run -p "Analyze AAPL momentum" --json
deep-trace run -f strategy.txt
echo "Backtest 000001.SZ RSI" | deep-trace run
```

```bash
deep-trace -p "your prompt"
deep-trace --skills
deep-trace --swarm-presets
deep-trace --swarm-run investment_committee '{"topic":"BTC outlook"}'
deep-trace --list
deep-trace --show <run_id>
deep-trace --code <run_id>
deep-trace --pine <run_id>           # Export indicators (TradingView + TDX + MT5)
deep-trace --trace <run_id>
deep-trace --continue <run_id> "refine the strategy"
deep-trace --upload report.pdf
```

```bash
deep-trace alpha list --zoo gtja191 --limit 10
deep-trace alpha show gtja191_171
deep-trace alpha bench --zoo gtja191 --universe csi300 --period 2018-2025 --top 20
```
...
deep-trace run -p "Backtest a 20/50-day moving average crossover on AAPL for the past year, show Sharpe ratio and max drawdown"

# RSI mean-reversion on crypto
deep-trace run -p "Test RSI(14) mean-reversion on BTC-USDT: buy below 30, sell above 70, last 6 months"

# Multi-factor strategy on A-shares
deep-trace run -p "Backtest a momentum + value + quality multi-factor strategy on CSI 300 constituents over 2 years"

# After backtesting, export to TradingView / TDX / MetaTrader 5
deep-trace --pine <run_id>
```
...
```bash
deep-trace alpha list --zoo gtja191 --theme reversal --limit 10
deep-trace alpha show gtja191_171
```
...
```bash
# Equity deep-dive
deep-trace run -p "Research NVDA: earnings trend, analyst consensus, option flow, and key risks for next quarter"

# Macro analysis
deep-trace run -p "Analyze the current Fed rate path, USD strength, and impact on EM equities and gold"

# Crypto on-chain
deep-trace run -p "Deep dive BTC on-chain: whale flows, exchange balances, miner activity, and funding rates"
```
...
deep-trace --swarm-run investment_committee '{"topic": "Is TSLA a buy at current levels?"}'

# Quant strategy from screening to backtest
deep-trace --swarm-run quant_strategy_desk '{"universe": "S&P 500", "horizon": "3 months"}'

# Crypto desk: funding + liquidation + flow → risk manager
deep-trace --swarm-run crypto_trading_desk '{"asset": "ETH-USDT", "timeframe": "1w"}'

# Global macro portfolio allocation
deep-trace --swarm-run macro_rates_fx_desk '{"focus": "Fed pivot impact on EM bonds"}'
```
...
deep-trace run -p "Remember: I prefer RSI-based strategies, max 10% drawdown, hold period 5–20 days"

# The agent recalls them in future sessions automatically
deep-trace run -p "Build a crypto strategy that fits my risk profile"
```
...
deep-trace --upload trades_export.csv
deep-trace run -p "Profile my trading behavior and identify any biases"

deep-trace --upload NVDA_Q1_earnings.pdf
deep-trace run -p "Summarize the key risks and beats/misses from this earnings report"
```
...
For localhost development, `deep-trace serve` keeps the browser workflow simple. For any non-local client, sensitive API endpoints require `API_AUTH_KEY`; use `Authorization: Bearer <key>` for JSON/upload requests. Browser EventSource streams are handled by the Web UI after you enter the same key once in Settings.

Shell-capable tools are available to local CLI and trusted localhost workflows, but are not exposed to remote API sessions unless you explicitly set `DEEP_TRACE_ENABLE_SHELL_TOOLS=1`. Document and journal readers are limited to upload/import roots by default; place files under `agent/uploads`, `agent/runs`, `./uploads`, `./data`, `~/.deep-trace/uploads`, or `~/.deep-trace/imports`, or add a dedicated directory through `DEEP_TRACE_ALLOWED_FILE_ROOTS`.
...
Deep-Trace exposes 22 MCP tools for any MCP-compatible client. Runs as a stdio subprocess — no server setup needed. **21 of 22 tools work with zero API keys** (HK/US/crypto). Only `run_swarm` needs an LLM key.
...
```bash
deep-trace-mcp                  # stdio (default)
deep-trace-mcp --transport sse  # SSE for web clients
```
...
Browse on ClawHub: [clawhub.ai/skills/deep-trace](https://clawhub.ai/skills/deep-trace)
...
```json
{
  "mcpServers": {
    "openspace": {
      ...
      "toolTimeout": 600,
      "env": {
        "OPENSPACE_HOST_SKILL_DIRS": "/path/to/deep-trace/agent/src/skills",
...
OpenSpace will auto-discover all 75 skills, enabling auto-fix, auto-improve, and community sharing. Search for Deep-Trace skills via `search_skills("finance backtest")` in any OpenSpace-connected agent.
...
> The MCP Plugin lets *other* agents call Deep-Trace tools.
> This section lets the *built-in* Deep-Trace agent call tools from *your* external MCP servers.
...
```bash
deep-trace run "use my-server to do X"
```
...
Config file location: `~/.deep-trace/agent.json` (JSON or YAML).
...
│   │   │   └── persistent.py       #   file-based memory (~/.deep-trace/memory/)
│   │   │
...
Deep-Trace is part of the **[HKUDS](https://github.com/HKUDS)** agent ecosystem:
...
> We ship in phases. Items move to [Issues](https://github.com/HKUDS/Deep-Trace/issues) when work begins.
...
**Good first issues** are tagged with [`good first issue`](https://github.com/HKUDS/Deep-Trace/issues?q=is%3Aissue+is%3Aopen+label%3A%22good+first+issue%22) — pick one and get started.
...
Thanks to everyone who has contributed to Deep-Trace!
...
<a href="https://github.com/HKUDS/Deep-Trace/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=HKUDS/Deep-Trace" />
</a>
...
Deep-Trace is for research, simulation, and backtesting only. It is not investment advice and it does not execute live trades. Past performance does not guarantee future results.
...
[![Star History Chart](https://api.star-history.com/svg?repos=HKUDS/Deep-Trace&type=Date)](https://star-history.com/#HKUDS/Deep-Trace&Date)

<p align="center">
  ⭐ If <b>Deep-Trace</b> helps your research, a star helps more people find it.
</p>

---

<p align="center">
  Thanks for visiting <b>Deep-Trace</b> ✨
</p>
<p align="center">
  <img src="https://visitor-badge.laobi.icu/badge?page_id=HKUDS.Deep-Trace&style=flat" alt="visitors"/>
</p>
