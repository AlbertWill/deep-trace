...
Triggered automatically when ``~/.deep-trace/.env`` does not exist. Five
back-steppable steps (provider → model → key → timeout → optional Tushare),
matching §3.2 of the 2026-05-19 UI/UX design proposal.

Each step persists immediately to ``~/.deep-trace/.env.partial`` and the
...
def _env_dir() -> Path: return Path.home() / ".deep-trace"
...
def _intro_header(console: Console) -> None:
    console.print()
    console.print(Text("  Deep-Trace setup", style=Theme.primary))
    console.print(Text(
        "  We didn't find a config at ~/.deep-trace/.env.\n"
    ...
    def step_key() -> object:
        ...
        while True:
            key = _prompt_secret(
                ...
                "(saved to ~/.deep-trace/.env, never logged)",
                console=cons,
            )
